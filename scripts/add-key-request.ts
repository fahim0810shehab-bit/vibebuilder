...
async function saveKey(token: string, moduleId: string, keyName: string, translations: Record<string, string>) {
  const payload = {
    keyName: keyName,
    moduleId: moduleId,
  ...
  if (!res.ok) {
    ...
    console.log(`[SUCCESS] ${keyName} saved and published.`);
  }
}
...
async function main() {
  try {
    ...
    // Saving the missing keys found in en-US.json
    await saveKey(token, moduleId, 'WELCOME_MESSAGE', {
      'en-US': 'Welcome to VibeBuilder!',
      'de-DE': 'Willkommen bei VibeBuilder!',
      'fr-FR': 'Bienvenue sur VibeBuilder !',
      'bn-BD': 'VibeBuilder এ আপনাকে স্বাগতম!'
    });

    await saveKey(token, moduleId, 'BTN_SUBMIT', {
      'en-US': 'Submit',
      'de-DE': 'Absenden',
      'fr-FR': 'Soumettre',
      'bn-BD': 'জমা দিন'
